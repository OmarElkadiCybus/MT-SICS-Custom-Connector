...
- Connection tuning knobs (in the connection config, with env fallbacks): `connectionStrategy` (backoff delays), `connectTimeoutMs` (TCP handshake bound), `connectValidationCommand` + `connectValidationTimeoutMs` (probe right after connect; set command to `none` to disable), and `tcpKeepaliveMs` (half-open detection).
- Queueing: commands are serialized (next command waits for response or timeout before sending). Each uses its own timeout; on timeout the socket is dropped and backoff reconnect kicks in.
- Logs: set `LOG_LEVEL=debug` to trace queueing, validation, timeouts, reconnects, and parsing. Simulator HTTP control logs connection resets as warnings only.
